    ...
    <main className='min-h-full w-full from-gray-900 to-gray-600 bg-gradient-to-r'>
      <header className='container text-center mx-auto py-3 px-4 min-w-full shadow-md shadow-bottom shadow-yellow-600 flex justify-between text-yellow-500'>
        <h1 className="flex justify-center items-center text-2xl font-extrabold   md:text-3xl lg:text-4xl text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-yellow-400">
          <svg class="w-8 h-8 text-yellow-600 mr-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 21 18">
      ...
      <AddAndEditModal isUnderEdit={isUnderEdit} setIsUnderEdit={setIsUnderEdit} listKey={listKey} />

      <section className='mt-8 flex justify-start gap-8'>
        <div className='border-b pb-2 border-gray-600 h-fit'>
          ...
          {
            Object.keys(shoppingList).length > 0 && Object.keys(shoppingList).map(key => {
              return (
                <div href="#" class="p-2 rounded-r-full hover:bg-gray-700" key={key} data-value={key} onClick={handleSetCurrentList} ref={selectedDivRef}>
                  <h5 class="text-lg font-bold w-fit tracking-tight text-gray-400 " data-value={key} onClick={handleSetCurrentList}>{shoppingList[key].name}</h5>
                  <p class="pl-3 font-normal w-fit text-gray-500" data-value={key} onClick={handleSetCurrentList}>{shoppingList[key].items.length} Items</p>
                </div>
              )
            })
          }
        ...
        </div>
        {
          ...
          <div className='flex-1 shadow-left shadow-yellow-600 pl-3'>

            <div>
              <h2 className="mb-4 text-2xl font-extrabold text-gray-900 dark:text-white md:text-lg lg:text-3xl text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">
                {currentList.name} list items</h2>
...

            {
              currentList.items.length > 0 ?
                          ...
                          return (
                            <>
                              <tr className="h-16 border border-red-500 rounded-lg pr-4 bg-slate-100 ">
                                ...
                                <td className="px-5">
                                  <div className="flex items-center" onClick={() => { setIsUnderEdit(true); toggleListModal() }}>
                ...
                <div className='h-full w-full flex flex-col justify-center items-center gap-3'>
                  <img src={catBoxImage} alt='cat in a box' className='w-40' loading='lazy'/>
                  ...
                  <button
                    ...
                    <svg class="w-6 h-6 text-yellow-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
